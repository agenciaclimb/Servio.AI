    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const suggestionResponse = await fetch(
          `${import.meta.env.VITE_AI_API_URL}/api/suggest-maintenance`,
          {
            method: 'POST',
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <button
        ...
        className="text-sm font-medium text-blue-600 hover:text-blue-500 mb-4"
      >
        &larr; Voltar
      </button>
      <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1">
            ...
            {item.serialNumber && (
              <p className="text-sm">
                <strong>Nº de Série:</strong> {item.serialNumber}
              </p>
            )}
          </div>
        </div>
        ...
        {suggestion && (
          <div className="mt-6 bg-blue-50 border-l-4 border-blue-400 p-4 rounded-r-lg">
            <h4 className="font-bold text-blue-800">
              💡 Sugestão da IA: {suggestion.suggestionTitle}
            </h4>
          ...
          <div className="space-y-4">
            {history.length > 0 ? (
              history.map(job => (
                ...
                >
                  ...
                  <p className="text-sm text-gray-500">
                    Concluído em: {new Date(job.completedAt!).toLocaleDateString()} por{' '}
                    {job.providerId}
                  </p>
                </Link>
              ))
              ...
              <p className="text-center text-gray-500 py-4">
                Nenhum serviço registrado para este item.
              </p>
            )}
          </div>
        </div>
