  ...
  describe('Queries', () => {
    beforeEach(async () => {
      ...
      await db
        .collection('jobs')
        .doc('job1')
        .set({ title: 'Limpeza', status: 'aberto', budget: 100 });
      await db
        .collection('jobs')
        .doc('job2')
        .set({ title: 'Pintura', status: 'aberto', budget: 200 });
      await db
        .collection('jobs')
        .doc('job3')
        .set({ title: 'Reparos', status: 'fechado', budget: 150 });
    });

    it('deve filtrar com where()', async () => {
      const snapshot = await db.collection('jobs').where('status', '==', 'aberto').get();
    ...
    it('deve ordenar com orderBy()', async () => {
      const snapshot = await db.collection('jobs').orderBy('budget', 'asc').get();
