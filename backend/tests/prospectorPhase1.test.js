...
function createMockDb(initialData = {}) {
  ...
  Object.entries(initialData).forEach(([coll, docs]) => {
    ...
    Object.entries(docs).forEach(([id, data]) => {
      collections[coll].set(id, { id, _data: { ...data }, ref: { update: (patch) => {
      ...
      return {
        doc(id) {
          return {
            ...
            async set(data) { coll.set(id, { id, _data: { ...data }, ref: { update: (patch) => {
...
describe('Prospector Phase 1 Endpoints', () => {
  const mockDb = createMockDb({
    prospectors: {
      'a@example.com': { name: 'Alice', totalRecruits: 12, activeRecruits: 10, totalCommissionsEarned: 120 },
      'b@example.com': { name: 'Bob', totalRecruits: 4, activeRecruits: 4, totalCommissionsEarned: 40 },
      'c@example.com': { name: 'Carol', totalRecruits: 30, activeRecruits: 28, totalCommissionsEarned: 450 }
    }
  });
  ...
  test('GET /api/prospector/stats returns correct badge and progress', async () => {
    const res = await request(app).get('/api/prospector/stats').query({ prospectorId: 'a@example.com' });
  ...
  test('GET /api/prospector/stats not found', async () => {
    const res = await request(app).get('/api/prospector/stats').query({ prospectorId: 'x@example.com' });
