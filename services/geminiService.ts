 ...
 * @throws Error após 3 tentativas falhas
 */
 ...
 */
export async function enhanceJobRequest(
  prompt: string,
  address?: string,
  fileCount?: number
): Promise<EnhancedJobRequest> {
  try {
    return await fetchFromBackend<EnhancedJobRequest>('/api/enhance-job', {
      prompt,
      address,
      fileCount,
    });
...
export async function generateProposalMessage(job: Job, provider: User): Promise<string> {
  const { message } = await fetchFromBackend<{ message: string }>('/api/generate-proposal', {
    job,
    provider,
  });
  return message;
...
export async function enhanceProviderProfile(profile: {
  name: string;
  headline: string;
  bio: string;
}): Promise<{ suggestedHeadline: string; suggestedBio: string }> {
  return fetchFromBackend<{ suggestedHeadline: string; suggestedBio: string }>(
    '/api/enhance-profile',
    { profile }
  );
}

export async function generateSEOProfileContent(
  user: User,
  reviews: Review[]
): Promise<SEOProfileContent> {
...
export async function summarizeReviews(providerName: string, reviews: Review[]): Promise<string> {
  const { summary } = await fetchFromBackend<{ summary: string }>('/api/summarize-reviews', {
    providerName,
    reviews,
  });
  return summary;
...
export async function generateReviewComment(
  ...
  const { comment } = await fetchFromBackend<{ comment: string }>('/api/generate-comment', {
    rating,
    category,
    description,
  });
...
export async function generateProfileTip(user: User): Promise<string> {
  ...
  const isVitest =
    typeof process !== 'undefined' && (process.env as Record<string, string | undefined>)?.VITEST;
  if (isVitest) {
...
export async function generateReferralEmail(
  ...
  return fetchFromBackend<{ subject: string; body: string }>('/api/generate-referral', {
    senderName,
    friendEmail,
  });
}

export async function generateCategoryPageContent(
  category: string,
  location?: string
): Promise<CategoryPageContent> {
  return fetchFromBackend<CategoryPageContent>('/api/generate-category-page', {
    category,
    location,
  });
}
...
export async function suggestMaintenance(
  item: MaintainedItem
): Promise<MaintenanceSuggestion | null> {
  return fetchFromBackend<MaintenanceSuggestion | null>('/api/suggest-maintenance', { item });
}

export async function proposeScheduleFromChat(
  messages: Message[]
): Promise<ScheduledDateTime | null> {
...
export async function getChatAssistance(
  ...
  return fetchFromBackend<ChatSuggestion | null>('/api/get-chat-assistance', {
    messages,
    currentUserType,
  });
}
...
export async function extractInfoFromDocument(
  base64Image: string,
  mimeType: string
): Promise<ExtractedDocumentInfo> {
  return fetchFromBackend<ExtractedDocumentInfo>('/api/extract-document', {
    base64Image,
    mimeType,
  });
}
...
export async function mediateDispute(
  ...
  clientName: string,
  providerName: string
): Promise<{ summary: string; analysis: string; suggestion: string }> {
  return fetchFromBackend<{ summary: string; analysis: string; suggestion: string }>(
    '/api/mediate-dispute',
    { messages, clientName, providerName }
  );
}

export async function analyzeProviderBehaviorForFraud(
  provider: User,
  context: { type: 'proposal' | 'bid' | 'profile_update'; data: Record<string, unknown> }
): Promise<{ isSuspicious: boolean; riskScore: number; reason: string } | null> {
  return fetchFromBackend<{ isSuspicious: boolean; riskScore: number; reason: string } | null>(
    '/api/analyze-fraud',
    { provider, context }
  );
}
