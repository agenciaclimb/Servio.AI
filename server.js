...
app.post('/api/enhance-job', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'enhance-job');
  }
});
...
app.post('/api/analyze-relevance', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'analyze-relevance');
  }
});
...
app.post('/api/match-providers', checkAuth, async (req, res) => {
  try {
    ...
    const scored = await Promise.all(
      providers.map(async provider => {
        const proximityScore =
          job.address &&
          provider.location &&
          job.address.includes((provider.location.split(',')[1] || '').trim())
            ? 100
            : 30;
        const availabilityScore = availabilityMap[job.urgency] || 50;
        const specialtyScore = (provider.specialties || []).some(s =>
          (job.category || '').toLowerCase().includes(String(s).toLowerCase())
        )
          ? 100
          : 0;
        const relevanceScore = await analyzeRelevanceScore(job.description, {
    ...
    handleApiError(res, error, 'match-providers');
  }
});
...
app.post('/api/generate-proposal', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'generate-proposal');
  }
});
...
app.post('/api/generate-faq', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'generate-faq');
  }
});
...
app.post('/api/identify-item', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'identify-item');
  }
});
...
app.post('/api/enhance-profile', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'enhance-profile');
  }
});
...
app.post('/api/generate-seo', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'generate-seo');
  }
});
...
app.post('/api/summarize-reviews', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'summarize-reviews');
  }
});
...
app.post('/api/generate-comment', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'generate-comment');
  }
});
...
app.post('/api/generate-tip', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'generate-tip');
  }
});
...
app.post('/api/generate-referral', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'generate-referral');
  }
});
...
app.post('/api/generate-category-page', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'generate-category-page');
  }
});
...
app.post('/api/suggest-maintenance', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'suggest-maintenance');
  }
});
...
app.post('/api/propose-schedule', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'propose-schedule');
  }
});
...
app.post('/api/get-chat-assistance', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'get-chat-assistance');
  }
});
...
app.post('/api/suggest-chat-reply', checkAuth, async (req, res) => {
  try {
    ...
    if (!messages || !currentUserType || messages.length === 0) {
      return res
        .status(400)
        .json({ error: 'Histórico de mensagens e tipo de usuário são obrigatórios.' });
    }
...
app.post('/api/analyze-sentiment', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'analyze-sentiment');
  }
});
...
app.post('/api/generate-earnings-tip', checkAuth, async (req, res) => {
  try {
    ...
    res.json({
      tip: (
        response.text ||
        'Concentre-se em oferecer um excelente atendimento para garantir avaliações de 5 estrelas. Isso não só atrai mais clientes, mas também aumenta seus ganhos!'
      ).trim(),
    });
  } catch (error) {
...
app.post('/api/parse-search', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'parse-search');
  }
});
...
app.post('/api/extract-document', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'extract-document');
  }
});
...
app.post('/api/mediate-dispute', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'mediate-dispute');
  }
});
...
app.post('/api/analyze-fraud', checkAuth, async (req, res) => {
  try {
    ...
    handleApiError(res, error, 'analyze-fraud');
  }
});
