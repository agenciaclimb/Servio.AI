...
export default function FollowUpSequences({
        ...
        const scheduleData = {
          ...
          steps: sequence.steps.map((step, idx) => ({
            ...
            scheduledFor: Timestamp.fromDate(
              new Date(Date.now() + step.dayOffset * 24 * 60 * 60 * 1000)
            ),
  ...
  return (
    <div className="fixed inset-0 bg-black/60 grid place-items-center z-[70]" onClick={onClose}>
      <div
        className="bg-white rounded-xl shadow-2xl w-[800px] max-w-[90vw] max-h-[85vh] overflow-auto p-6"
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-600">
              Follow-up inteligente para {selectedLeads.length} lead(s) selecionado(s)
            </p>
          </div>
        ...
        {selectedSequence && (
          <div className="mt-4 bg-blue-50 border-2 border-blue-200 rounded-lg p-3 text-sm text-blue-800">
            <strong>💡 Dica:</strong> Os follow-ups serão personalizados automaticamente com o nome
            e categoria de cada lead. Você pode acompanhar o progresso no dashboard.
          </div>
        )}
      </div>
    </div>
