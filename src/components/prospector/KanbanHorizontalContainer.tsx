...
export default function KanbanHorizontalContainer({
  headers,
  children,
}: KanbanHorizontalContainerProps) {
  return (
    <div className="w-full">
      <div className="sticky top-0 z-10 bg-white/80 backdrop-blur border-b">
        <div className="flex gap-4 px-4 py-3 overflow-x-auto">
          {headers.map(h => (
            <div
              key={h.id}
              className={`min-w-[260px] shrink-0 px-3 py-2 rounded-md border ${h.color ?? ''}`}
            >
      ...
      <div className="flex gap-4 px-4 py-4 overflow-x-auto">{children}</div>
    </div>
  );
}
