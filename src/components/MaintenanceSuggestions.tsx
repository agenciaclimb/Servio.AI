  ...
  if (isLoading) {
      return (
          <div className="bg-slate-100 p-4 rounded-lg text-center">
              <p className="text-sm text-gray-600 animate-pulse">Buscando sugestões inteligentes para seus itens...</p>
          </div>
      )
  }
    ...
    <div className="bg-gradient-to-r from-indigo-50 via-purple-50 to-pink-50 p-6 rounded-2xl shadow-sm border">
        ...
        <div className="flex space-x-4 overflow-x-auto pb-4 -mb-4">
            {suggestions.map((suggestion) => (
                <SuggestionCard key={suggestion.item.id} suggestion={suggestion} onSuggestJob={onSuggestJob} />
            ))}
        </div>
    </div>
  );
