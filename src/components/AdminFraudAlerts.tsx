...
const AdminFraudAlerts: React.FC<AdminFraudAlertsProps> = ({ allAlerts, allUsers, setAllAlerts }) => {
  ...
  if (allAlerts.length === 0) {
    return <div className="text-center bg-white p-10 rounded-lg shadow-sm border">
        ...
        <p className="mt-1 text-sm text-gray-500">Nenhum alerta de fraude foi gerado.</p>
    </div>;
  }
            ...
            <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {allAlerts.map(alert => {
                        ...
                        return (
                            <tr key={alert.id}>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{new Date(alert.createdAt).toLocaleString('pt-BR')}</td>
