...
export async function trackStageChange(campaignId: string | null, from: string, to: string) {
  ...
  await setDoc(metricsRef, { count: increment(1), updatedAt: serverTimestamp(), campaignId }, { merge: true });
}

export async function trackChannelAction(channel: 'whatsapp' | 'email' | 'call', success = true) {
  const date = new Date().toISOString().slice(0, 10);
  const metricsRef = doc(db, `prospector_metrics/${date}`, `channel_${channel}_${success ? 'success' : 'fail'}`);
