...
vi.mock('../components/ProviderOnboarding', () => ({ default: ({ onComplete }: any) => <div data-testid="onboarding">Onboarding <button onClick={onComplete}>fin</button></div> }));
vi.mock('../components/ProviderDashboardSkeleton', () => ({ default: () => <div data-testid="skeleton">Loading...</div> }));
vi.mock('../components/ProfileStrength', () => ({ default: ({ onEditProfile }: any) => <button data-testid="profile-strength" onClick={onEditProfile}>Strength</button> }));
vi.mock('../components/ProfileTips', () => ({ default: () => <div data-testid="profile-tips"/> }));
vi.mock('../components/ReferralProgram', () => ({ default: ({ onSendReferral }: any) => <button onClick={() => onSendReferral('amigo@ex.com')} data-testid="referral-btn">Refer</button> }));
vi.mock('../components/PaymentSetupCard', () => ({ default: () => <div data-testid="payment-setup"/> }));
vi.mock('../components/ProviderEarningsCard', () => ({ default: () => <div data-testid="earnings"/> }));
vi.mock('../components/ProposalModal', () => ({ default: ({ onClose, onSubmit }: any) => <div data-testid="proposal-modal"><button onClick={()=>{onSubmit({message:'Olá', price:100}); onClose();}}>Enviar</button><button onClick={onClose}>Fechar</button></div> }));
vi.mock('../components/AuctionRoomModal', () => ({ default: ({ onClose }: any) => <div data-testid="auction-modal"><button onClick={onClose}>Sair</button></div> }));
vi.mock('../components/ProfileModal', () => ({ default: ({ onSave }: any) => <div data-testid="profile-modal"><button onClick={()=>onSave({headline:'Nova', bio:'Bio nova'})}>Salvar Perfil</button></div> }));
vi.mock('../components/ReferralInvitationModal', () => ({ default: ({ onClose }: any) => <div data-testid="referral-modal"><button onClick={onClose}>Fechar Ref</button></div> }));
vi.mock('../components/ChatModal', () => ({ default: ({ onSendMessage, onConfirmSchedule, job }: any) => <div data-testid="chat-modal"><button onClick={()=>onSendMessage({ chatId: job.id, text:'Ping'})}>Msg</button><button onClick={()=>onConfirmSchedule(job.id,{date:'2025-12-01', time:'10:00'})}>Agendar</button></div> }));
vi.mock('../components/ProviderJobCard', () => ({ default: ({ job, onUpdateStatus, onChat }: any) => <div data-testid={`provider-job-${job.id}`}>JOB {job.id}<button onClick={()=>onUpdateStatus(job.id, job.status==='agendado'?'a_caminho':'agendado')}>Avançar</button><button onClick={onChat}>Chat</button></div> }));
vi.mock('../components/JobCard', () => ({ default: ({ job, onProposeClick }: any) => <div data-testid={`job-${job.id}`}><span>{job.category}</span><button onClick={onProposeClick}>Propor</button></div> }));
...
vi.mock('../components/useProviderDashboardData', () => ({
    useProviderDashboardData: () => ({
        data: {
            availableJobs: [
                { id:'j1', category:'limpeza', status:'aberto', clientId:'cli@ex.com', jobMode:'normal' },
                { id:'j2', category:'reparos', status:'aberto', clientId:'cli@ex.com', jobMode:'leilao' }
            ],
            myJobs: [
                { id:'mj1', category:'limpeza', status:'agendado', clientId:'cli@ex.com', jobMode:'normal' }
            ],
            completedJobs: [],
            myProposals: [],
            myBids: [],
...
vi.mock('../services/geminiService', () => ({
  ...
  generateReferralEmail: vi.fn(async (name, email)=> ({ subject:'Convite', body:`Olá ${email}` })),
}));
