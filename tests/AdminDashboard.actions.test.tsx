...
vi.mock('../components/AdminAnalyticsDashboard', () => ({
  default: () => <div data-testid="tab-content-analytics">Analytics</div>,
}));
vi.mock('../components/AdminJobManagement', () => ({
  default: ({ onMediateClick }: any) => (
    <div data-testid="tab-content-jobs">
      <button
        onClick={() =>
...
vi.mock('../components/AdminProviderManagement', () => ({
  default: () => <div data-testid="tab-content-providers">Providers</div>,
}));
vi.mock('../components/AdminFinancials', () => ({
  default: () => <div data-testid="tab-content-financials">Financials</div>,
}));
vi.mock('../components/AdminFraudAlerts', () => ({
  default: () => <div data-testid="tab-content-fraud">Fraud</div>,
}));
...
vi.mock('../services/api', () => ({
  ...
  fetchUserById: vi.fn(async (email: string) => ({
    email,
    name: email.includes('cli') ? 'Cliente' : 'Prestador',
    type: email.includes('cli') ? 'cliente' : 'prestador',
  })),
