...
test.describe('[E2E] Cliente - Fluxos críticos', () => {
  test('criar job simples e visualizar na lista', async ({ page, loginAsClient }) => {
    await loginAsClient();
    await page.getByRole('button', { name: /solicitar serviço|criar serviço|novo job/i }).first().click();

    const wizard = page.getByRole('dialog', { name: /criar serviço|solicitar serviço/i });
    await wizard.getByLabelText(/descrição|o que você precisa/i).fill('Instalar tomadas em 2 quartos');
  ...
  test('abrir disputa a partir de job em andamento (happy path visual)', async ({ page, loginAsClient }) => {
    await loginAsClient();
    ...
    await page.getByRole('button', { name: /problema|abrir disputa|contestar/i }).first().click();
    const modal = page.getByRole('dialog', { name: /disputa|problema/i });
    await modal.getByLabelText(/motivo|descrição/i).fill('Fluxo E2E: qualidade abaixo do esperado.');
