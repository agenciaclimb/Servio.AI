...
test.describe('E-commerce Navigation and UI', () => {
  test('should navigate to products page from header', async ({ page }) => {
    ...
    const hasLoadingOrContent = await page
      .locator('.loading-container, .products-grid, .products-list, .empty-cart, .no-products')
      .isVisible()
      .catch(() => false);
  ...
  test('should have order tracking page accessible', async ({ page }) => {
    ...
    if (hasOrderTracking) {
      ...
      const hasOrderTrackingUI = await page
        .locator('.search-form, .order-item, .no-orders, .tracking-timeline')
        .isVisible()
        .catch(() => false);
  ...
  test('should render ProductListing component with proper structure', async ({ page }) => {
    ...
    const hasViewToggle = await page
      .locator('.view-btn, [data-testid="view-toggle"]')
      .isVisible()
      .catch(() => false);
    const hasSortSelect = await page
      .locator('.sort-select, [data-testid="sort-select"]')
      .isVisible()
      .catch(() => false);
    const hasFiltersSidebar = await page
  ...
  test('should show empty cart message when no items', async ({ page }) => {
    ...
    if (await cartButton.isVisible()) {
      ...
      const hasEmptyMessage = await page
        .locator('.empty-cart, .no-items, text=/carrinho vazio/i')
        .isVisible()
        .catch(() => false);
  ...
  test('should render checkout form with multiple steps', async ({ page }) => {
    ...
    if (hasCheckoutRoute) {
      ...
      const hasProgressBar = await page
        .locator('.progress-bar, [data-testid="progress-bar"]')
        .isVisible()
        .catch(() => false);
      const hasForm = await page
        .locator('form, [data-testid="checkout-form"]')
        .isVisible()
        .catch(() => false);
      const hasLoginPrompt = await page
        .locator('button', { hasText: 'Fazer Login' })
        .isVisible()
        .catch(() => false);
