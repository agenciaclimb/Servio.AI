...
export const test = base.extend<RoleFixtures>({
  ...
  loginAsProvider: async ({ page }, use) => {
    await use(async () => {
      ...
      await page
        .getByRole('dialog')
        .filter({ hasText: /bem-vindo de volta/i })
        .waitFor();
      await page.getByLabel(/email/i).fill('e2e-prestador@servio.ai');
      await page.getByLabel(/senha/i).fill('SenhaE2E!123');
      await page.getByTestId('auth-submit-button').click();
      await page
        .getByText(/dashboard|prestador|meus jobs|painel/i)
        .first()
        .waitFor({ timeout: 10000 });
    });
  },
  ...
  loginAsAdmin: async ({ page }, use) => {
    await use(async () => {
      ...
      await page.getByTestId('auth-submit-button').click();
      await page
        .getByText(/admin|painel.*administrativo|dashboard/i)
        .first()
        .waitFor({ timeout: 10000 });
    });
  },
  ...
  loginAsProspector: async ({ page }, use) => {
    await use(async () => {
      ...
      await page.getByTestId('auth-submit-button').click();
      await page
        .getByText(/prospector|crm|pipeline/i)
        .first()
        .waitFor({ timeout: 10000 });
    });
  },
