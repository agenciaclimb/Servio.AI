...
const ProviderOnboarding: React.FC<ProviderOnboardingProps> = ({ user, onComplete }) => {
  const [step, setStep] = useState<
    'welcome' | 'upload' | 'loading' | 'review' | 'pending' | 'rejected'
  >('welcome');
  const [error, setError] = useState<string | null>(null);
  const [documentImage, setDocumentImage] = useState<string | null>(null); // base64 data URL
  const [extractedData, setExtractedData] = useState<ExtractedDocumentInfo>({
    fullName: '',
    cpf: '',
  });
        ...
        return (
          <div className="text-center">
            <h2 className="text-2xl font-bold text-gray-800">
              Bem-vindo(a) à SERVIO.AI, {user.name}!
            </h2>
            <p className="mt-2 text-gray-600">
              Para garantir a segurança de todos, precisamos verificar sua identidade. O processo é
              rápido e simples.
            </p>
            <button
              onClick={() => setStep('upload')}
              className="mt-6 px-8 py-3 text-lg font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700"
            >
              Começar Verificação
            </button>
          </div>
        ...
        return (
          <div className="text-center">
            ...
            <p className="mt-2 text-gray-600 text-sm">
              Por favor, envie uma foto nítida da frente do seu RG ou CNH.
            </p>
        ...
        return (
          <div>
            ...
            <p className="mt-2 text-gray-600 text-sm text-center">
              Nossa IA extraiu estas informações. Por favor, verifique se estão corretas.
            </p>
            {documentImage && (
              <img
                src={documentImage}
            ...
            <button
              ...
              className="mt-6 w-full px-8 py-3 font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700"
            >
              Enviar para Análise
            </button>
          </div>
      ...
      case 'pending':
        return (
          <div className="text-center">
            ...
            <p className="mt-2 text-gray-600">
              Recebemos seus documentos! Nossa equipe irá analisá-los em breve. Você será notificado
              assim que o processo for concluído (geralmente em até 24h).
            </p>
          </div>
        );
      case 'rejected':
        return (
          <div className="text-center">
            ...
            <p className="mt-2 text-gray-600">
              Houve um problema ao verificar seus documentos. Isso pode acontecer se a imagem
              estiver ilegível ou se os dados não puderem ser confirmados.
            </p>
            <button
              onClick={() => setStep('upload')}
              className="mt-6 px-8 py-3 text-lg font-medium text-white bg-red-600 rounded-md hover:bg-red-700"
            >
              Tentar Novamente
            </button>
          </div>
