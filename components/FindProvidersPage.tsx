...
const FindProvidersPage: React.FC<FindProvidersPageProps> = ({ allUsers, allJobs, onViewProfile, onContact }) => {
  ...
  const providersWithStats = useMemo(() => {
    return allUsers
      ...
      .map(user => {
        const providerJobs = allJobs.filter(j => j.providerId === user.email && j.status === 'concluido' && j.review);
  ...
  const filteredAndSortedProviders = useMemo(() => {
    const results = providersWithStats.filter(user => {
      ...
      const locationMatch = !filters.location || user.location.toLowerCase().includes(locationLower);
      ...
      const availabilityMatch = filters.availability === 'any' || user.availability === filters.availability;
    ...
    switch (sortBy) {
        ...
        default:
            ...
            results.sort((a, b) => {
                const scoreA = (a.headline?.toLowerCase().includes(filters.service.toLowerCase()) ? 5 : 0) + a.stats.jobsCompleted + (a.stats.avgRating * 2);
                const scoreB = (b.headline?.toLowerCase().includes(filters.service.toLowerCase()) ? 5 : 0) + b.stats.jobsCompleted + (b.stats.avgRating * 2);
  ...
  return (
    <div className="space-y-8">
      <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
        <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Encontre o Profissional Perfeito</h1>
        <p className="mt-2 text-lg text-gray-600">Use nossa busca inteligente ou os filtros para achar exatamente o que você precisa.</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <aside className="lg:col-span-1 bg-white p-6 rounded-2xl shadow-lg border border-gray-100 self-start">
            <div className="flex justify-between items-center">
                ...
                <button onClick={clearFilters} className="text-xs font-semibold text-blue-600 hover:underline">Limpar</button>
            </div>
            <div className="space-y-6 mt-4">
                <div>
                    <label htmlFor="service" className="block text-sm font-medium text-gray-700">Serviço / Nome</label>
                    <input type="text" name="service" id="service" value={filters.service} onChange={handleFilterChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm" />
                </div>
                 <div>
                    <label htmlFor="location" className="block text-sm font-medium text-gray-700">Localização</label>
                    <input type="text" name="location" id="location" value={filters.location} onChange={handleFilterChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm" />
                </div>
                <div>
                    <label htmlFor="availability" className="block text-sm font-medium text-gray-700">Disponibilidade</label>
                    <select name="availability" id="availability" value={filters.availability} onChange={handleFilterChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                ...
                <div className="space-y-3">
                    <div className="relative flex items-start">
                        <div className="flex h-6 items-center"><input id="isVerified" name="isVerified" type="checkbox" checked={filters.isVerified} onChange={handleFilterChange} className="h-4 w-4 rounded border-gray-300" /></div>
                        <div className="ml-3 text-sm"><label htmlFor="isVerified" className="font-medium text-gray-800">Identidade Verificada</label></div>
                    </div>
                     <div className="relative flex items-start">
                        <div className="flex h-6 items-center"><input id="hasCertificates" name="hasCertificates" type="checkbox" checked={filters.hasCertificates} onChange={handleFilterChange} className="h-4 w-4 rounded border-gray-300" /></div>
                        <div className="ml-3 text-sm"><label htmlFor="hasCertificates" className="font-medium text-gray-800">Possui Certificados</label></div>
                    </div>
                </div>
            </div>
        </aside>
        ...
        <main className="lg:col-span-3">
            <div className="flex flex-col sm:flex-row justify-between items-center mb-4">
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <select id="sort-by" value={sortBy} onChange={e => setSortBy(e.target.value as SortBy)} className="rounded-md border-gray-300 shadow-sm text-sm">
