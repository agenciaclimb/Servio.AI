      ...
      >
        <div className="relative p-8 text-center">
          ...
          <p className="text-gray-600 mt-2">
            Você está contratando <span className="font-semibold">{provider.name}</span> para o
            serviço de <span className="font-semibold">{job.category}</span>.
          </p>
          ...
          <div className="mt-6 text-left bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <h4 className="font-bold text-yellow-800 text-sm mb-2">
            ...
            <ul className="space-y-1 text-xs text-yellow-700 list-disc list-inside">
              ...
              <li>
                Você aprova a liberação para o profissional somente após a conclusão do serviço.
              </li>
          ...
          {error && (
            <div
              className="mt-4 text-left p-3 rounded-lg border"
            ...
            >
              ...
              {(errorCode === 'E_TIMEOUT' || errorCode === 'E_NETWORK') && (
                <p className="text-xs mt-1">
                  Conexão instável ou tempo esgotado. Verifique sua internet e tente novamente.
                </p>
              )}
            </div>
          )}
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3">
            <button
              onClick={onClose}
              disabled={isLoading}
              className="flex-1 px-4 py-3 text-sm font-medium rounded-md bg-gray-200 text-gray-800 hover:bg-gray-300"
            >
              Cancelar
            </button>
            <button
              ...
              disabled={isLoading}
              className="flex-1 px-4 py-3 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:bg-blue-300 flex items-center justify-center"
            >
              {isLoading ? (
                <>
                  <svg
              ...
              ) : error && (errorCode === 'E_TIMEOUT' || errorCode === 'E_NETWORK') ? (
                'Tentar novamente'
              ) : (
                'Pagar com Stripe'
              )}
