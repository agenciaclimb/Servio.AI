...
import { Job, DisputeMessage, User, Dispute } from '../types';
...
const DisputeModal: React.FC<DisputeModalProps> = ({ user, job, dispute, otherParty, onClose, onSendMessage }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4" aria-modal="true" role="dialog" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg m-4 transform transition-all h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <header className="relative p-4 border-b border-red-200 bg-red-50 flex-shrink-0">
            ...
            <p className="text-sm text-red-600 truncate">Job: {job.category} com {otherParty?.name || 'Usuário'}</p>
        </header>
        
        <div className="p-4 border-b border-gray-200 bg-slate-50 text-center">
            <p className="text-sm text-gray-700">O pagamento está <span className="font-bold">pausado</span> até que a disputa seja resolvida. Mantenha a comunicação registrada aqui.</p>
        </div>

        <main className="flex-grow overflow-y-auto p-4 space-y-4 bg-slate-100">
            {dispute.messages.map(msg => (
                <div key={msg.id} className={`flex ${msg.senderId === user.email ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-xs md:max-w-md px-4 py-2 rounded-2xl ${msg.senderId === user.email ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800'}`}>
                        <p>{msg.text}</p>
                        <p className={`text-xs mt-1 ${msg.senderId === user.email ? 'text-blue-200' : 'text-gray-500'}`}>
        ...
        <footer className="p-4 border-t border-gray-200 flex-shrink-0">
            <form onSubmit={handleSubmit} className="flex items-center space-x-3">
                ...
                />
                <button type="submit" className="inline-flex items-center justify-center h-10 w-10 rounded-full bg-red-600 text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
