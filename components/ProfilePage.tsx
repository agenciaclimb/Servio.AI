...
const ProfilePage: React.FC<ProfilePageProps> = ({
  ...
  const providerJobs = user
    ? allJobs.filter(
        job => job.providerId === user.email && job.status === 'concluido' && job.review
      )
    : [];
        ...
        <div className="p-8">
            ...
            <div className="mt-4 sm:mt-0 sm:ml-6">
              ...
              {user.type === 'prestador' && (
                <p className="text-lg text-blue-600 font-semibold">
                  {isSeoLoading ? 'Carregando...' : seoContent?.publicHeadline || user.headline}
                </p>
              )}
          ...
          <div className="mt-8">
            ...
            <p className="mt-2 text-gray-600 whitespace-pre-wrap">
              {isSeoLoading
                ? 'Carregando...'
                : seoContent?.publicBio || user.bio || 'Nenhuma biografia fornecida.'}
            </p>
          </div>
          ...
          {user.type === 'prestador' && (
            <div className="mt-8">
              ...
              <LocationMap
                locations={[{ id: user.email, name: user.location, type: 'provider' }]}
              />
            </div>
          )}
        ...
        {user.type === 'prestador' && providerJobs.length > 0 && (
          <div className="bg-slate-50 p-8 border-t border-gray-200">
            <h2 className="text-xl font-bold text-gray-800 mb-6">
              Histórico de Trabalho e Avaliações
            </h2>
