...
const AuctionRoomModal: React.FC<AuctionRoomModalProps> = ({
  job,
  currentUser,
  bids: initialBids,
  onClose,
  ...
  const handlePlaceBid = (e: React.FormEvent) => {
    ...
    if (lowestBid && amount >= lowestBid) {
      setError(
        `Seu lance deve ser menor que ${lowestBid.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}.`
      );
      return;
    }
    setError('');
  ...
  return (
    ...
    >
      <div
        ...
        className="bg-white rounded-2xl shadow-xl w-full max-w-3xl m-4 transform transition-all h-[90vh] flex flex-col"
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 flex-grow min-h-0">
          ...
          <main className="md:col-span-2 flex-grow overflow-y-auto p-6 space-y-4 bg-slate-50 border-r">
            ...
            {sortedBids.length > 0 ? (
              sortedBids.map((bid, index) => (
                <div
                  key={bid.id}
                  className={`flex items-center p-3 rounded-lg ${index === sortedBids.length - 1 ? 'bg-green-100 border border-green-200' : 'bg-white'}`}
                >
                  ...
                  <div className="ml-3 flex-grow">
                    <p className="text-sm font-semibold text-gray-700">
                      {bidderAnonymizationMap.get(bid.providerId) || 'Lance Anônimo'}
                    </p>
          ...
          <aside className="p-6 flex flex-col justify-between">
            <div>
              ...
              <div className="pt-6">
                <h4 className="text-sm font-semibold text-gray-600 text-center mb-1">
                  Menor Lance
                </h4>
              ...
              <form onSubmit={handlePlaceBid} className="mt-6">
                ...
                <div className="relative mt-1 rounded-md shadow-sm">
                  ...
                  <input
                    ...
                    className="block w-full rounded-md border-gray-300 pl-10 pr-4 py-2"
                ...
                <button
                  type="submit"
                  className="w-full mt-3 px-4 py-3 text-sm font-medium text-white bg-blue-600 border rounded-lg hover:bg-blue-700"
                >
                  Confirmar Lance
                </button>
              </form>
