...
const serviceTypeDetails: {
  [key in ServiceType]: { text: string; className: string; icon: string };
} = {
...
const JobCard: React.FC<JobCardProps> = ({ job, bids, onProposeClick, hasProposed }) => {
  ...
  const serviceType =
    (job && job.serviceType && serviceTypeDetails[job.serviceType as ServiceType]) ||
    defaultServiceType;
  const isAuction = job.jobMode === 'leilao';

  const lowestBid = isAuction && bids.length > 0 ? Math.min(...bids.map(b => b.amount)) : null;
      ...
      <div className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-200 flex flex-col h-full transform hover:-translate-y-1 transition-transform duration-300">
        <div className="p-6 flex-grow flex flex-col">
          <div className="flex justify-between items-start">
            ...
            <span
              className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${isAuction ? 'bg-orange-100 text-orange-800' : serviceType.className}`}
            >
              {isAuction ? (
                '⚖️ Leilão'
              ) : (
                <>
                  {serviceType.icon} <span className="ml-1.5">{serviceType.text}</span>
                </>
              )}
            </span>
          </div>
            ...
            job.fixedPrice && (
              <div className="mt-4 text-center bg-green-50 p-3 rounded-lg border border-green-200">
                ...
                <p className="text-2xl font-bold text-green-700">
                  {job.fixedPrice.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                </p>
              </div>
            )
          )}

          <p className="mt-4 text-gray-700 text-sm flex-grow line-clamp-4">{job.description}</p>
        </div>
