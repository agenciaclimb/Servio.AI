...
const ProspectorStatistics: React.FC<{ prospectorId: string }> = ({ prospectorId }) => {
  ...
  useEffect(() => {
    if (
      ...
      stats.currentBadge !== prevBadgeRef.current
    ) {
  ...
  return (
    <>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard
          label="Comissões (R$)"
          value={stats?.totalCommissionsEarned?.toFixed(2) || '0.00'}
        />
        <StatCard
          label="Média Comissão"
          value={stats?.averageCommissionPerRecruit?.toFixed(2) || '0.00'}
        />
      </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <div className="text-xl font-semibold text-purple-600">
              {badge.nextBadge || 'Máximo alcançado'}
            </div>
          </div>
        </div>
        <ProgressBar value={badge.progressToNextBadge} />
        <div className="text-xs text-gray-500">
          Progresso: {badge.progressToNextBadge.toFixed(1)}% para {badge.nextBadge}
        </div>
        <div className="flex gap-2 flex-wrap text-xs">
          {badge.tiers.map(t => (
            <span
              key={t.name}
              className={`px-2 py-1 rounded border ${badge.currentBadge === t.name ? 'bg-indigo-50 border-indigo-300 text-indigo-700' : 'bg-gray-50 border-gray-200 text-gray-600'}`}
            >
              {t.name} ≥ {t.min}
            </span>
          ))}
      ...
      <div className="bg-white p-5 rounded border shadow-sm mt-4">
        <h2 className="text-lg font-semibold mb-4 text-gray-800">
          Leaderboard (Top 10 por Comissões)
        </h2>
