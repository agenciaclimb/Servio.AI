...
const ChatModal: React.FC<ChatModalProps> = ({
  ...
  const scrollToBottom = () => {
    ...
    if (
      el &&
      typeof (el as unknown as { scrollIntoView?: unknown }).scrollIntoView === 'function'
    ) {
  ...
  const handleSuggestSchedule = (e: React.FormEvent) => {
    ...
    const formattedDate = new Date(`${scheduleDate}T00:00:00`).toLocaleDateString('pt-BR', {
      weekday: 'long',
      day: '2-digit',
      month: 'long',
    });
      ...
      >
        <header className="relative p-4 border-b border-gray-200 flex-shrink-0">
          ...
          <h2 className="text-lg font-bold text-gray-800">
            Chat com {otherParty?.name || 'Usuário'}
          </h2>
        ...
        <main className="flex-grow overflow-y-auto p-4 space-y-4 bg-slate-100">
          {messages.map(msg => {
            ...
            if (msg.type === 'schedule_proposal' && msg.schedule) {
              ...
              const formattedDate = new Date(`${msg.schedule.date}T00:00:00`).toLocaleDateString(
                'pt-BR',
                ...
                >
                  <div className="max-w-xs md:max-w-md p-3 rounded-2xl bg-white border border-indigo-200 shadow-sm">
                    <p className="text-sm font-semibold text-indigo-800">
                      🗓️ Proposta de Agendamento
                    </p>
                    <p className="text-sm text-gray-700 mt-2">
                      Para: <strong>{formattedDate}</strong>
                    </p>
                    <p className="text-sm text-gray-700">
                      Às: <strong>{msg.schedule.time}</strong>
                    </p>
                    {isRecipient && !isConfirmed && job.status !== 'agendado' && (
                      <div className="mt-3 pt-2 border-t border-indigo-100 flex space-x-2">
                        <button
                          onClick={() => onConfirmSchedule(job.id, msg.schedule!, msg.id)}
                          className="flex-1 px-3 py-1 text-xs font-bold text-white bg-green-600 rounded-md hover:bg-green-700"
                        >
                          Confirmar
                        </button>
                      </div>
            ...
            return (
              ...
              >
                <div
                  className={`max-w-xs md:max-w-md px-4 py-2 rounded-2xl ${isCurrentUser ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800'}`}
                >
                  <p>{msg.text}</p>
                  <p
                    className={`text-xs mt-1 ${isCurrentUser ? 'text-blue-200' : 'text-gray-600'}`}
                  >
        ...
        <footer className="p-4 border-t border-gray-200 flex-shrink-0 space-y-3">
          ...
          {assistantSuggestion && (
            <div className="p-2 bg-indigo-50 border border-indigo-200 rounded-lg flex items-center justify-between">
              ...
              <button
                onClick={() => handleSuggestionClick(assistantSuggestion)}
          ...
          {isScheduling && (
            <form
              onSubmit={handleSuggestSchedule}
              className="p-3 bg-slate-50 border border-gray-200 rounded-lg space-y-2"
            >
              <div className="flex items-center gap-3">
                <input
                  ...
                  required
                  className="flex-1 block w-full text-sm p-2 border border-gray-300 rounded-md"
                />
                <input
                  type="time"
                  value={scheduleTime}
          ...
          <form onSubmit={handleSubmit} className="flex items-center space-x-2">
            ...
            <button
              ...
              className="flex-shrink-0 inline-flex items-center justify-center h-10 w-10 rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200"
            >
              <svg
                className="w-5 h-5"
                xmlns="http://www.w3.org/2000/svg"
              ...
              >
                <path
                  fillRule="evenodd"
                  d="M5.75 3a.75.75 0 01.75.75v.5h7V3.75a.75.75 0 011.5 0V4h.25A2.75 2.75 0 0118 6.75v8.5A2.75 2.75 0 0115.25 18H4.75A2.75 2.75 0 012 15.25v-8.5A2.75 2.75 0 014.75 4H5v-.25A.75.75 0 015.75 3zM4.5 8.25v7a1.25 1.25 0 001.25 1.25h9.5A1.25 1.25 0 0016.5 15.25v-7h-12z"
                  clipRule="evenodd"
                />
              </svg>
            </button>
            <button
              ...
              title="Assistente IA"
              className="flex-shrink-0 inline-flex items-center justify-center h-10 w-10 rounded-full bg-indigo-100 text-indigo-600 hover:bg-indigo-200 disabled:opacity-50"
            >
              {isAssistantLoading ? (
                <svg
            ...
            <button
              type="submit"
              className="flex-shrink-0 inline-flex items-center justify-center h-10 w-10 rounded-full bg-blue-600 text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
            >
              <svg
