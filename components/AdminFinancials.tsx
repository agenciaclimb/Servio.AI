...
const AdminFinancials: React.FC = () => {
  ...
  const completedJobProposals = allProposals.filter(p =>
    completedJobs.some(j => j.id === p.jobId && p.status === 'aceita')
  );
  ...
  const totalInEscrow = allEscrows
    .filter(e => e.status === 'bloqueado' || e.status === 'em_disputa')
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        ...
        />
        <StatCard
          title="Total em Custódia (Escrow)"
      ...
      <div className="bg-white shadow-sm border rounded-lg overflow-hidden">
        <h3 className="text-lg font-semibold text-gray-800 p-4 border-b">
          Histórico de Transações de Custódia (Escrow)
        </h3>
                ...
                return (
                  <tr key={escrow.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                      {new Date(escrow.createdAt).toLocaleDateString('pt-BR')}
                    </td>
                  </tr>
                );
              })}
