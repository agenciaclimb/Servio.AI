...
const SitemapGenerator: React.FC<SitemapGeneratorProps> = ({
  ...
  const serviceLocationUrls = Array.from(serviceLocations).map(
    sl =>
      `${baseUrl}/servico/${encodeURIComponent(sl.split('/')[0])}/${encodeURIComponent(sl.split('/')[1])}`
  );
      ...
      >
        <header className="relative p-6 border-b border-gray-200">
          ...
          <p className="text-sm text-gray-600 mt-1">
            Esta é uma lista de todas as URLs importantes que seriam enviadas ao Google.
          </p>
        </header>
