    ...
    <div className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-200 flex flex-col h-full">
        ...
        {job.address && (
          <div className="mt-2 flex items-center text-xs text-gray-600">
            ...
            >
              <path
                fillRule="evenodd"
                d="M9.69 18.933l.003.001C9.89 19.02 10 19 10 19s.11.02.308-.066l.002-.001.006-.003.018-.008a5.741 5.741 0 00.281-.14c.186-.1.4-.22.655-.364C11.888 18.01 12.872 17.5 14 16.5c1.128-1 2-2.304 2-3.5V6.5a1 1 0 00-1-1h-1.414a1 1 0 00-.707.293L10 8.586 7.121 5.707A1 1 0 006.414 5.5H5a1 1 0 00-1 1v6.5c0 1.196.872 2.5 2 3.5 1.128 1 2.112 1.51 2.692 1.933z"
                clipRule="evenodd"
              />
            </svg>
      ...
      <div className="px-6 pb-4 pt-4 bg-gray-50 border-t border-gray-100 space-y-2">
        ...
        {job.status === 'em_leilao' && (
          <button
            onClick={onViewProposals}
            className="w-full text-center py-2 px-4 border rounded-lg text-sm font-medium transition duration-300 border-transparent text-white bg-orange-600 hover:bg-orange-700"
          >
            Acompanhar Leilão ⚖️
          </button>
        )}
        {(job.status === 'proposta_aceita' ||
          job.status === 'agendado' ||
          ...
          <div className="flex space-x-2">
            ...
            <button
              ...
              title="Ver no Mapa"
              className="flex-shrink-0 py-2 px-3 border rounded-lg text-sm font-medium transition duration-300 border-gray-300 bg-white text-gray-700 hover:bg-gray-50"
            >
              <svg
                className="w-5 h-5"
                xmlns="http://www.w3.org/2000/svg"
              ...
              >
                <path
                  fillRule="evenodd"
                  d="M9.69 18.933l.003.001C9.89 19.02 10 19 10 19s.11.02.308-.066l.002-.001.006-.003.018-.008a5.741 5.741 0 0 0 .281-.14c.186-.1.4-.22.655-.364C11.888 18.01 12.872 17.5 14 16.5c1.128-1 2-2.304 2-3.5V6.5a1 1 0 0 0-1-1h-1.414a1 1 0 0 0-.707.293L10 8.586 7.121 5.707A1 1 0 0 0 6.414 5.5H5a1 1 0 0 0-1 1v6.5c0 1.196.872 2.5 2 3.5 1.128 1 2.112 1.51 2.692 1.933z"
                  clipRule="evenodd"
                />
              </svg>
            </button>
        ...
        {job.status === 'pagamento_pendente' && (
          <>
            <button
              onClick={onFinalize}
              className="w-full text-center py-2 px-4 border rounded-lg text-sm font-medium transition duration-300 border-transparent text-white bg-green-600 hover:bg-green-700"
            >
              Finalizar e Avaliar
            </button>
            <button
              onClick={onReportIssue}
              className="w-full text-center py-2 px-4 mt-2 border rounded-lg text-sm font-medium transition duration-300 border-gray-300 bg-white text-red-600 hover:bg-red-50"
            >
        ...
        {job.status === 'em_disputa' && (
          <button
            onClick={onChat}
            className="w-full text-center py-2 px-4 border rounded-lg text-sm font-medium transition duration-300 border-transparent text-white bg-red-600 hover:bg-red-700"
          >
            Ver Disputa
          </button>
        )}
          ...
          <div className="text-center">
            ...
            <div className="flex items-center justify-center space-x-1">
              {Array.from({ length: 5 }, (_, i) => (
                <svg
                  key={i}
                  className={`w-4 h-4 ${i < job.review!.rating ? 'text-yellow-400' : 'text-gray-300'}`}
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
