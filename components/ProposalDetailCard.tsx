...
const statusDetails: {
  [key in Proposal['status']]: { text: string; bg: string; text_color: string; border: string };
} = {
  pendente: {
    text: 'Pendente',
    bg: 'bg-yellow-50',
...
const ProposalDetailCard: React.FC<ProposalDetailCardProps> = ({
  proposal,
  provider,
  onAccept,
  isJobDecided,
  ...
  return (
    <div
      className={`rounded-xl border ${status.border} ${status.bg} p-5 transition-all duration-300 ${proposal.status !== 'pendente' ? 'opacity-75' : ''}`}
    >
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between">
        <div className="flex-grow">
          <div className="flex items-center justify-between">
            ...
            <span
              className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${status.bg} ${status.text_color}`}
            >
              {status.text}
            </span>
          </div>
