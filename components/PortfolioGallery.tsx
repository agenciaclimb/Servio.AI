...
const PortfolioLightbox: React.FC<{ item: PortfolioItem; onClose: () => void }> = ({
  item,
  onClose,
}) => (
  <div
  ...
  >
    <div
      className="bg-white rounded-lg shadow-xl w-full max-w-3xl m-4 transform transition-all max-h-[90vh] flex flex-col"
    ...
    >
      <img
        src={item.imageUrl}
        alt={item.title}
        className="w-full h-auto object-contain rounded-t-lg flex-grow"
    ...
    <>
      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {items.map(item => (
          <div
            key={item.id}
            className="group relative cursor-pointer aspect-w-4 aspect-h-3"
            onClick={() => setSelectedItem(item)}
            onKeyDown={handleKeyDown(() => setSelectedItem(item))}
            tabIndex={0}
            role="button"
