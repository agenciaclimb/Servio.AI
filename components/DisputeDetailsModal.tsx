...
const DisputeDetailsModal: React.FC<DisputeDetailsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4">
      <div
        className="bg-white rounded-2xl shadow-xl w-full max-w-2xl m-4 h-[90vh] flex flex-col"
      ...
      >
        ...
        <main className="flex-grow overflow-y-auto p-6 space-y-4 bg-slate-50">
          {dispute.messages.map(msg => (
            ...
            >
              <div
                className={`max-w-md p-3 rounded-lg ${msg.senderId === currentUser.email ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800'}`}
              >
                <p className="text-xs font-bold mb-1">{getSenderName(msg.senderId)}</p>
                <p className="text-sm">{msg.text}</p>
                <p
                  className={`text-xs mt-1 text-right ${msg.senderId === currentUser.email ? 'text-blue-200' : 'text-gray-500'}`}
                >
