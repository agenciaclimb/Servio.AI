...
const AdminJobManagement: React.FC<AdminJobManagementProps> = ({ onMediateClick }) => {
  ...
  const filteredJobs = (filter === 'all' ? allJobs : allJobs.filter(j => j.status === filter)).sort(
      ...
      {totalPages > 1 && (
        <div className="px-4 py-3 bg-gray-50 border-t flex items-center justify-between">
          <div className="text-sm text-gray-700">
            ...
            {Math.min(page * ITEMS_PER_PAGE, filteredJobs.length)} de {filteredJobs.length} jobs
          </div>
