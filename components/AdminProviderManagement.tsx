...
const AdminProviderManagement: React.FC = () => {
  ...
  const handleVerificationDecision = async (
    userId: string,
    decision: 'verificado' | 'recusado'
  ) => {
    // optimistic update
    const previousUsers = allUsers;
    setAllUsers(prev =>
      prev.map(u => (u.email === userId ? { ...u, verificationStatus: decision } : u))
    );
                    ...
                    return (
                      <tr key={provider.email}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          {provider.verificationStatus === 'verificado' && (
                            <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                              Verificado
                            </span>
                          )}
                          {provider.verificationStatus === 'recusado' && (
                            <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">
                              Recusado
                            </span>
                          )}
                          {provider.verificationStatus === 'pendente' && (
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${statusStyle?.bg || 'bg-gray-100'} ${statusStyle?.text || 'text-gray-800'}`}
                          >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-4">
                          ...
                          {provider.status === 'ativo' ? (
                            <button
                              onClick={() => handleUpdateStatus(provider.email, 'suspenso')}
                            ...
                            <button
                              onClick={() => handleUpdateStatus(provider.email, 'ativo')}
            ...
            {totalPages > 1 && (
              <div className="px-4 py-3 bg-gray-50 border-t flex items-center justify-between">
                <div className="text-sm text-gray-700">
                  ...
                  {Math.min(page * ITEMS_PER_PAGE, providers.length)} de {providers.length}{' '}
                  prestadores
                </div>
