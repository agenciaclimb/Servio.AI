...
const AdminDisputeModal: React.FC<AdminDisputeModalProps> = ({
  job,
  dispute,
  client,
  provider,
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl shadow-xl w-full max-w-4xl m-4 transform transition-all h-[90vh] flex flex-col"
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 flex-grow min-h-0">
          ...
          <div className="flex flex-col border-r border-gray-200">
            <h3 className="p-3 text-sm font-semibold text-gray-700 bg-slate-100 border-b">
            ...
            <main className="flex-grow overflow-y-auto p-4 space-y-4 bg-slate-50">
              {dispute.messages.map(msg => (
                <div
                  key={msg.id}
                  className={`flex ${msg.senderId === client.email ? 'justify-start' : 'justify-end'}`}
                >
                  <div
                    className={`max-w-xs md:max-w-md px-4 py-2 rounded-2xl ${msg.senderId === client.email ? 'bg-gray-200 text-gray-800' : 'bg-blue-600 text-white'}`}
                  >
                    <p className="font-bold text-xs mb-1">
                      {msg.senderId === client.email ? client.name : provider.name}
                    </p>
                    <p>{msg.text}</p>
                  </div>
                </div>
          ...
          <div className="flex flex-col">
            ...
            <div className="flex-grow overflow-y-auto p-4 space-y-4">
              {!aiAnalysis && !isLoadingAnalysis && (
                <button
                  ...
                  className="w-full flex justify-center items-center py-3 px-6 border border-transparent rounded-lg shadow-sm text-lg font-medium text-white bg-blue-600 hover:bg-blue-700"
                >
                  Analisar com IA ✨
                </button>
              )}
              {isLoadingAnalysis && (
                <p className="text-center animate-pulse">Analisando disputa...</p>
              )}
              {aiAnalysis && (
                <div className="space-y-4 text-sm">
                  <div>
                    <h4 className="font-bold text-gray-800">Resumo da IA</h4>
                    <p className="mt-1 text-gray-600 bg-gray-50 p-3 rounded-md border">
                      {aiAnalysis.summary}
                    </p>
                  </div>
                  <div>
                  ...
                  <div className="!mt-6">
                    <h4 className="font-bold text-blue-800 bg-blue-50 p-2 rounded-md">
                      Sugestão da IA
                    </h4>
                    <p className="mt-1 text-blue-700 font-semibold p-3 border-l-4 border-blue-500">
            ...
            <footer className="p-4 border-t border-gray-200 flex-shrink-0 bg-gray-50 space-x-4 text-right">
              <button
                onClick={() => handleResolution('reembolsado')}
                className="px-4 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md shadow-sm hover:bg-red-700"
              >
                Reembolsar Cliente
              </button>
              <button
                onClick={() => handleResolution('liberado')}
