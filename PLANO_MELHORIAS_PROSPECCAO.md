...
exports.autoProspectCron = functions.pubsub
...
exports.followUpExecutor = functions.pubsub
...
function QuickAddPanel() {
  ...
  const handlePaste = async () => {
    ...
    const enriched = await Promise.all(
      parsed.map(lead => enrichLeadData(lead))
    );
...
class AIAutopilot {
  ...
  async execute(prospectorId) {
    ...
    for (const job of jobsNeedingHelp) {
      ...
      const enriched = await Promise.all(
        filtered.map(p => this.enrichWithAI(p, job))
      );
